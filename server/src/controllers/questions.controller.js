...
//  Get ALL student questions (including deleted) 
export async function getStudentQuestionsAll(req, res) {
try {
const { rows: headers } = await pool.query(
"SELECT id, header, identifier FROM headers ORDER BY id ASC"
);
...
const { rows: questions } = await pool.query(
"SELECT id, questions, header_id, header_version FROM questions WHERE header_id = ANY($1::int[]) AND header_version = ANY($2::text[])",
[headerIds, identifiers]
);
...
try {
const { rows: headers } = await pool.query(
"SELECT id, header FROM header_t ORDER BY id ASC"
);
